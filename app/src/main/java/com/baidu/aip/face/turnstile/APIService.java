...
public class APIService {
     ...
     * 一个多脸的图片，去一个库中搜索
     */
    private static final String MULTI_IDENTIFY_URL = BASE_URL + "/rest/2.0/face/v3/multi-search";
    ...
    public void identify(OnResultListener listener, File file,boolean isMultiFace) {
        RegParams params = new RegParams();
        String base64Img = "";
        try {
            byte[] buf = readFile(file);
        ...
        String url = "";
        if(isMultiFace){
            url = urlAppendCommonParams(MULTI_IDENTIFY_URL);
        }else{
            url = urlAppendCommonParams(IDENTIFY_URL);
        }
